            ...
            pkg.forEach(function(file){
                ...
                if(ret.map.res[id]){
                    ...
                    if(c != ''){
                        ...
                        /**
                        * 修改打包文件内部资源地址的相对路径。
                        * 打包后的文件  pkg/common.js
                        * 打包的文件    modules/xxx/xxx.js 
                        * @date  2015-4-15
                        * @author  rsk
                        */
                        if(fis.config.get('roadmap.relative') && file._likes.isCssLike){
                            c = (function(file){
                                return c.replace(/\burl\s*\(\s*("(?:[^\\"\r\n\f]|\\[\s\S])*"|'(?:[^\\'\n\r\f]|\\[\s\S])*'|[^)}\s]+)\s*\)(\s*;?)/g, function(m, url){
                                    var rPath = fis.util.getRelativePath(fis.util.getAbsolutePath(url, file.url), pkgUrl);
                                    return 'url(' + rPath + ')';
                                });
                            })(file);
                        }
